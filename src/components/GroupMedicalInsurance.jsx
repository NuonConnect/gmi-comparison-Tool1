...
const TAKAFUL_EMARAT_TEMPLATES = {
  ...
  'Blue 2202': {
    // Blue 2202 uses a dedicated fixed comparison layout (Version 1 fields only).
    // Key order below drives the on-screen "Editable Template Preview" grid order.
    // NOTE: opticalDiscount is intentionally removed and `premium` is intentionally
    // omitted so the Average Premium is entered manually (not prefilled). Do not
    // re-add them without also updating the Blue 2202 render paths.
    planName: 'Blue 2202',
    network: 'E Care Blue',
    accessForOP: 'Only Clinics & Medical Centers',
    geographicalScope: 'UAE (Excluding the Emirate of Abu Dhabi & Al Ain Region). Emergency extension to UAE; Home country (IP only at ISC + SEA excluding China, Japan, HK, Taiwan, Thailand, Singapore)',
    ...
    physiotherapySessions: 'Covered up to 6 sessions with NIL Co-Pay',
    maternity: 'As per Std. DHA',
    dentalDiscounts: 'Covered up to limit of AED 500/- with 30% Co-insurance on all OP services.'
  },
  'Blue 2203': {
    planName: 'Blue 2203',
...
const SIMPLIFIED_TEMPLATE = {
  ...
  'ECARE Blue': {
  planName: 'ECARE Blue',
  annualLimit: 'Covered up to AED 150,000',
  geographicalScope: 'UAE (Excluding the Emirate of Abu Dhabi & Al Ain Region). Emergency extension to UAE; Homecountry (IP only at ISC + SEA excluding China, Japan, HK, Taiwan, Thailand, Singapore)',
  network: 'Blue',
  accessForOP: 'Only Clinics & Medical Centers',
  referralProcedure: 'GP referral to SP',
  ipCoinsurance: 'Covered with NIL Co-Pay',
  deductibleConsultation: 'Covered with 20% Co-Pay (Max AED 25)',
  opCoinsurance: 'Covered with NIL Co-Pay',
  pharmacyLimit: 'Covered up to AED 5,000',
  pharmacyCoinsurance: 'Covered with NIL Co-Pay',
  physiotherapySessions: 'Covered up to 6 sessions with NIL Co-Pay',
  maternity: 'As per Standard DHA',
  dental: 'Covered up to limit of AED 500/- with 30% Co-insurance on all OP services.'
}
...
// ============================================================================
// BLUE 2202 (Takaful Emarat - ECare) DEDICATED COMPARISON LAYOUT
// This is intentionally isolated so it does NOT affect any other provider,
// template or plan, or the shared comparison generation logic.
// ============================================================================

// Detect a Takaful Emarat "Blue 2202" plan. Uses the flag set at creation time,
// with a fallback to the stored product name so saved/reloaded plans still match.
const isBlue2202Plan = (plan) => {
  if (!plan) return false;
  if (plan.isBlue2202) return true;
  const pn = plan?.categoriesData?.planName;
  const name = pn && typeof pn === 'object' ? Object.values(pn)[0] : pn;
  return typeof name === 'string' && name.trim() === 'Blue 2202';
};
...
// Labels for the on-screen "Editable Template Preview" grid (Blue 2202 only).
const BLUE_2202_FIELD_LABELS = {
  planName: 'Product Name',
  network: 'Network',
  accessForOP: 'Access to OP',
  geographicalScope: 'Area of Cover',
  annualLimit: 'Annual Limit',
  ipCoinsurance: 'IP Co-insurance',
  referralProcedure: 'Referral Procedure - OP',
  deductibleConsultation: 'Deductible (Consultation)',
  opCoinsurance: 'OP Co-insurance',
  pharmacyLimit: 'Pharmacy Limit',
  pharmacyCoinsurance: 'Pharmacy Co-insurance',
...
const hasDHAManualPlan = plans.some(plan => plan.planType === 'DHA_MANUAL');
  ...
  // becomes PRODUCT. Every plan (Blue 2202 or another insurer) is mapped into the
  // same rows/columns via generateMergedRow. No separate HTML/CSS is produced and
  // no other insurer/layout is affected.
  const blue2202Present = plans.length > 0 && plans.some(isBlue2202Plan);
  const blue2202TbodyRows = !blue2202Present ? '' : (() => {
    const colCount = plans.length + 1;
    const infoCell = (plan, text) =>
      '<td style="text-align: center; white-space: pre-line;" class="' +
      (plan.id === highlightedPlanId ? 'benefit-cell highlighted' : '') + '">' + text + '</td>';
    ...
    const separatorRow = `<tr><th colspan="${colCount}">PLAN SUMMARY</th></tr>`;
    ...
    // DETAILS" section (further below) is rendered for Blue 2202 too, so the
    // exact same premium calculation/markup is reused (no duplication).
    return infoRows + separatorRow + benefitRows;
  })();
...
${!hideBrokerBranding ? `
        ...
        <table>
            <thead>
                <tr>
                    <th style="width: 200px; min-width: 200px;">${blue2202Present ? 'PRODUCT' : 'BENEFITS'}</th>
${plans.map((plan, index) => {
    ...
    <th class="plan-header" style="vertical-align: middle;">
        ...
        ${plan.id === highlightedPlanId ? '<div class="tag tag-highlighted">HIGHLIGHTED</div>' : ''}
            ...
            <tbody>
                ${blue2202Present ? blue2202TbodyRows : `
...
` : ''}
        ...
        ${plans.some(plan => plan.categoriesData?.prescribedPhysiotherapy) ? `
                ...
                `).join('')}
                `}
${blue2202Present ? `<tr><th colspan="${plans.length + 1}">PREMIUM DETAILS</th></tr>` : `<tr class="section-header">
...
// Helpers for the Saved History date filters
const isSameMonth = (d, ref) =>
  d.getFullYear() === ref.getFullYear() && d.getMonth() === ref.getMonth();

const isPreviousMonth = (d, ref) => {
...
const HistoryManager = ({ isOpen, onClose, history, onLoadComparison, onDeleteComparison, onNewComparison, onBackToNormal, onRefresh }) => {
  ...
  // Count-based stats (independent of the active filters)
  const totalSaved = history.length;
  const thisMonthCount = useMemo(
    () => history.filter((item) => item.date && isSameMonth(new Date(item.date), now)).length,
    [history]
  );
  const lastMonthCount = useMemo(
    () => history.filter((item) => item.date && isPreviousMonth(new Date(item.date), now)).length,
    [history]
  );

  // Distinct advisors for the dropdown
  ...
  const filteredHistory = useMemo(() => {
    return history.filter((item) => {
      ...
      if (advisorFilter !== 'all') {
        const name = item.createdBy || 'Unknown';
        if (name !== advisorFilter) return false;
      }
      return true;
    });
  ...
  const statCards = [
    { label: 'TOTAL SAVED', value: totalSaved, box: 'bg-indigo-50 border-indigo-100', text: 'text-indigo-700' },
    { label: 'THIS MONTH', value: thisMonthCount, box: 'bg-green-50 border-green-100', text: 'text-green-700' },
    { label: 'LAST MONTH', value: lastMonthCount, box: 'bg-amber-50 border-amber-100', text: 'text-amber-700' },
    { label: 'SHOWING', value: filteredHistory.length, box: 'bg-gray-50 border-gray-200', text: 'text-gray-700' },
  ];

  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ${isOpen ? '' : 'hidden'}`}>
      <div className="bg-white rounded-xl p-6 max-w-4xl w-full max-h-[85vh] overflow-y-auto">
        {/* Header */}
        <div className="flex justify-between items-start mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Saved History</h2>
            <p className="text-sm text-gray-500 mt-1">👥 Shared across all advisors</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={handleRefresh}
              disabled={isRefreshing}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-blue-700 transition text-sm disabled:opacity-60"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          {statCards.map((card) => (
            <div key={card.label} className={`border rounded-lg px-4 py-3 ${card.box}`}>
              <div className="text-[11px] font-semibold tracking-wide text-gray-500">{card.label}</div>
        ...
        {/* Filters */}
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-5 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <div>
              ...
              <select
                ...
                className="border border-gray-300 rounded-md px-3 py-2 text-sm bg-white min-w-[140px] focus:outline-none focus:ring-2 focus:ring-indigo-300"
              >
            ...
            <div>
              ...
              <select
                ...
                className="border border-gray-300 rounded-md px-3 py-2 text-sm bg-white min-w-[220px] focus:outline-none focus:ring-2 focus:ring-indigo-300"
              >
          ...
          <div className="text-sm text-gray-500 md:text-right">
            <span className="font-bold text-gray-800">{filteredHistory.length}</span> comparison{filteredHistory.length === 1 ? '' : 's'} in total
          </div>
        </div>
